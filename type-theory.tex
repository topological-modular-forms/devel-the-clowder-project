...
\begin{Proof}{Proof of \cref{constant-functions}}%
    ...
    \begin{scalewebprooftree}%
        \begin{prooftree}%
            ...
            \infer1[(λ-eq)]{\context{\Gamma}{\llbracket x\mapsto g(\const_{y}(x))\rrbracket\doteq\llbracket x\mapsto g(y)\rrbracket\oftype A\to B}}%
            \hypo{\context{\Gamma}{\const_{g(y)}\oftype A\to C}}%
            \infer1[(η)]{\context{\Gamma}{\llbracket x\mapsto g(y)\rrbracket\doteq\const_{g(y)}\oftype A\to B}}%
            \infer2[(trans)]{\context{\Gamma}{g\circ\const_{y}\doteq\const_{g(y)}\oftype A\to B}}%
        \end{prooftree}%
        .%
    \end{scalewebprooftree}%
    Now, for the proof of the judgement, we have
