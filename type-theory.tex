...
\begin{Proof}{Proof of \cref{properties-of-exponentiation}}%
    ...
    Omitted.
\end{Proof}
\subsection{The Well-Ordering Principle of the Natural Numbers}\label{subsection-the-well-ordering-principle-of-the-natural-numbers}
\begin{definition}{Bounds For Type Families Over $\N$}{bounds-for-type-families-over-n}%
    ...
    \begin{enumerate}
        \item\label{bounds-for-type-families-over-n-lower-bounds}A natural number $n$ is a \index[type-theory]{natural numbers!lower bound for a type family over}\textbf{lower bound} for $P$ if there exists a term of type%
            %--- Begin Footnote ---%
            \footnote{%
                \SloganFont{Intuition: }A lower bound for $P$ is a natural number $n$ such that if $P(x)$ holds, then so does $n\leq_{\N}x$.
            }%
            %---  End Footnote  ---%
            \[
                \islowerbound_{P}(n)%
            ...
            \]%
        \item\label{bounds-for-type-families-over-n-upper-bounds}A natural number $n$ is a \index[type-theory]{natural numbers!upper bound for a type family over}\textbf{upper bound} for $P$ if there exists a term of type
            \[
                \isupperbound_{P}(n)%
                \defeq%
                \prod_{\chigh{x\oftype\N}}\chigh{P(x)\to(x\leq_{\N}n)}.%
            \]%
        \item\label{bounds-for-type-families-over-n-minimal-terms}A natural number $n$ is a \index[type-theory]{natural numbers!minimal term for a type family over}\textbf{minimal term} for $P$ if there exists a term of type
            %--- Begin Footnote ---%
            \footnote{%
                \SloganFont{Intuition: }Informally, a minimal term for $P$ is, if it exists, the smallest natural number $n$ such that $P(n)$ holds.
                \par\vspace*{\TCBBoxCorrection}
            }%
            %---  End Footnote  ---%
            \[
            ...
            \]%
        \item\label{bounds-for-type-families-over-n-maximal-terms}A natural number $n$ is a \index[type-theory]{natural numbers!maximal term for a type family over}\textbf{maximal term} for $P$ if there exists a term of type
            \[
    ...
    \end{enumerate}
\end{definition}
\begin{theorem}{The Well Ordering Principle of the Natural Numbers}{the-well-ordering-principle-of-the-natural-numbers}%
    Let $P$ be a decidable type family indexed over $\N$, with $d$ a witness of $P$'s decidability. There exists a function%
    %--- Begin Footnote ---%
    \footnote{%
        \SloganFont{Slogan: }If $P$ is decidable, then either $P=\EmptyType$ or $P$ has a minimal term.
        \par\vspace*{\TCBBoxCorrection}
    }%
    %---  End Footnote  ---%
    \[
        ...
        \item\label{explicit-inference-rules-for-product-types-induction}\SloganFont{Induction. }We have
            \begin{webprooftree}%
                \begin{prooftree}%
                    ...
                    \infer3[(×-ind)]{\context{\Gamma}{\displaystyle\ind_{\mathord{\times}}\oftype\left(\prod_{x\oftype A}\prod_{y\oftype B}P(\pair(x,y))\right)\to\left(\prod_{z\oftype A\times B}P(z)\right)}}%
                \end{prooftree}%
                .%
            \end{webprooftree}%
...
\begin{remark}{Explicit Inference Rules for Identity Types}{explicit-inference-rules-for-identity-types}%
    ...
    \end{enumerate}
\end{remark}
\begin{intuition}{Intuition for Path Induction}{intuition-for-path-induction}%
    We may illustrate path induction using groupoids as follows, following \cite{MSE4386675}. Given a groupoid $\CatFont{G}$, we may form the arrow category $\Arr(\CatFont{G})$ of $\CatFont{G}$, whose objects may be thought of as \textit{paths in $\CatFont{G}$}. Namely, this is the category where:
    \begin{itemize}
    ...
    \]%
    given on objects by $(A,B,\phi)\mapsto(A,B)$ and on morphisms by $(f,g)\mapsto(f,g)$, as well as the composite functor
    \[
        \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={5.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
            \Arr(\CatFont{G})%
            \arrow[r,"\Wasureru"]
    ...
    \[
        ...
        \end{tikzcd}
    \]%
    of $\pr$ over $X$ is then (up to isomorphism) the coslice category $\CatFont{G}_{X/}$ of $\CatFont{G}$ under $X$. Namely, this is the category where:
    \begin{itemize}
    ...
    \[
        ...
        \end{tikzcd}
    \]%
    Since $\CatFont{G}$ is a groupoid, this means $(X,\id_{X})$ is also terminal. Thus, there exists a unique path
    \[
        \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={5.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
            \&
            X
        ...
        \end{tikzcd}
    \]%
    between any two objects $(A,\phi)$ and $(B,\psi)$ of $\CatFont{G}_{X/}$. As a result, $\CatFont{G}_{X/}$ is contractible, so given a functor
    \[
        F%
        \colon%
        \CatFont{G}_{X/}%
        ...
        \CatFont{H}%
    \]%
    from $\CatFont{G}_{X/}$ to another groupoid $\CatFont{H}$, it suffices to specify the image of $(X,\id_{X})$, as we can then define
    \begin{align*}
    ...
    \begin{itemize}
        \item An assignment $(A,\id_{X})\mapsto F(X,\id_{X})$ and outputs an assignment  $(A,\phi)\mapsto F(A,\phi)$.
    ...
    \begin{center}
        \begin{tabular}{|Sc|Sc|Sc|}\hline\rowcolor{darkRed}
            \textcolor{white}{\textbf{Setting}}    & \textcolor{white}{\textbf{Input}} & \textcolor{white}{\textbf{Output}} \\\hline\rowcolor{backgroundColor}
            Groupoids                              & $F(X,\id_{X})$                    & $F(A,\phi)$                        \\\rowcolor{black!05!backgroundColor}
