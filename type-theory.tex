...
\begin{intuition}{Intuition for Path Induction}{intuition-for-path-induction}%
    Given that groupoids form a categorical semantics for Martin-Löf type theory (\cite{the-groupoid-interpretation-of-type-theory}), we may wonder what path induction looks like in that setting. In this vein, we may illustrate path induction using groupoids as follows, following \cite{MSE4386675}. 
    ...
    \begin{webcompile}
        \pr^{-1}(X)%
        \defeq%
        \Arr{\CatFont{G}}\ttimes_{\CatFont{G}}\PunctualCategory,%
        \qquad%
        \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={5.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
            {\pr^{-1}(X)}
            \arrow[r]
