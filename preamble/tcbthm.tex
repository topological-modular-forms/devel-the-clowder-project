...
\Newtcbtheorem{remColor}{Remark}{Remarks}{tcbintuition}{intuition}
\Newtcbtheorem{remColor}{Remarks}{Remarks}{tcbintuitions}{intuitions}
...
\newenvironment{dnote}[1]{%
    $\mlp{\smash{\shiftbox{-1.0cm}{\raisebox{-0.625cm}{\huge\textdbend}}}}$%
    $\smash{\mlp{%
        \shiftbox{-4.5cm}{%
            \raisebox{1.5cm}{%
...
{
    \hfill%$\mrp{\smash{\phantom{a}}}$\newline\hfill
    $\mrp{\smash{\shiftbox{5.0cm}{\raisebox{-0.5cm}{$\mlp{%
        \begin{tikzpicture}[opacity=0.1, overlay, anchor=base]
...
%--- Proof Box
\definecolor{ProofBoxColor}{rgb}{0.25,0.25,0.25}
\definecolor{SubProofBoxColor}{rgb}{0.4,0.4,0.4}
\definecolor{SubSubProofBoxColor}{rgb}{0.55,0.55,0.55}
...
\newcommand{\FirstProofBox}[1]{%
    ...
    \noindent\proofBox{%
        \hyperwhite%
        $\mrp{\phantom{\text{ÂhỵyẮ}}}$% Fix height to descender + max(ascender,caps)
        #1%\mdseries\textsf{#1}%
        \hspace{\textwidth}%
...
\newcommand{\SubProofBox}[1]{\begin{subproofbox}\hypersetup{hidelinks}\centering{$\mrp{\phantom{\text{ÂhỵyẮ}}}$#1}\end{subproofbox}}% The purpose of ÂhỵyẮ is to fix height to descender + max(ascender,caps)
\newtcolorbox{subsubproofbox}{
  standard jigsaw,opacityback=0, % Transparent background
  coltext=white,        % Text color
  fontupper=\bfseries,  % Bold text
  boxrule=0pt,          % No border
  frame code={
    \path[fill=SubSubProofBoxColor]
      ([xshift=0.75em,yshift=0.75em]frame.north west) --  % Top-left, shifted right
      ([xshift=-0.75em,yshift=0.75em]frame.north east) -- % Top-right
      ([yshift=-0.25em]frame.north east) --               % Top-right
      ([xshift=-0.75em,yshift=0.25em]frame.south east) -- % Bottom-right, shifted left
      ([xshift=0.75em,yshift=0.25em]frame.south west) --  % Bottom-left
      ([yshift=1.25em]frame.south west) --                % Bottom-left
      cycle;
  },
}
\newcommand{\SubSubProofBox}[1]{\begin{subsubproofbox}\hypersetup{hidelinks}\centering{$\mrp{\phantom{\text{ÂhỵyẮ}}}$#1}\end{subsubproofbox}}% The purpose of ÂhỵyẮ is to fix height to descender + max(ascender,caps)
